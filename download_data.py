...
def download_clip(video_identifier, output_filename,
    ...
    command = ['ffmpeg_sources/ffmpeg/ffmpeg',
               '-i', '"%s"' % tmp_filename,
               '-ss', str(start_time),
...
def trim_dataset(df, percent):
    """ Utility method by @dbthaker to trim labels and reduce size of dataset.
    
    For each label, retain x% of the training data
    """
    grouped = df.groupby(['label-name'])
    resampled = grouped.apply(pd.DataFrame.sample, frac=percent).reset_index(drop=True)
    return resampled
...
def main(input_csv, output_dir,
    ...
    if num_jobs==1:
        ...
        status_lst = Parallel(n_jobs=num_jobs, verbose=51)(delayed(download_clip_wrapper)(
            row, label_to_dir,
