...
#-----------------------------------------------------------------------------
#
#SBATCH -J alex_pretrained                           # Job name
#SBATCH -o job_outputs/alex_pretrained-%j.out                     # Name of stdout output file (%j expands to jobId)
...
python3 one_shot_learning.py --dataset_type=kinetics_dynamic --controller_type=alex --batch_size=16 --image_width=128  --image_height=128 --seq_length=100 --summary_writer=True --model_saver=False --debug=True --memory_size=128 --memory_vector_dim=40 --n_classes=25 --class_difficulty=all --use_pretrained=True --num_epoches=1000 --rnn_size=200
